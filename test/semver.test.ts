...
describe("semver", () => {
    ...
    describe("addBranchPreRelease", () => {
        ...
        it("adds timestamp with sanitized branch", () => {
            ...
            if (db === da) {
                ...
                assert(p === `1.2.3-branch-some-crazy-branch-name-.${db}` || p === `1.2.3-branch-some-crazy-branch-name-.${da}`);
            }
        });
    ...
    describe("isMilestoneOrReleaseCandidate", () => {
        ...
        it("returns true for milesone release versions", () => {
            ["0.0.0-M.1", "0.1.0-M.42", "1.0.0-M.7", "1.2.3-M.5", "3.2.1-M.9", "12345.67890.1234567890-M.11"].forEach(v => {
                assert(isMilestoneOrReleaseCandidate(v));
            });
        });
